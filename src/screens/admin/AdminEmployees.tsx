...
// ── UserCard fuera del componente para evitar remounts ───────────────────────
...
export default function AdminEmployees() {
  ...
  const handleSave = () => {
    ...
    else { addUser({ ...form, isOnline: false }); }
    setModal(false);
  };
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => <UserCard user={item} onEdit={openEdit} onDelete={handleDelete} />}
        ListEmptyComponent={
      ...
      <Modal visible={modal} animationType="slide" transparent presentationStyle="overFullScreen">
        <View style={styles.overlay}>
          ...
          {/* FIX: KeyboardAvoidingView pega el sheet al teclado sin espacio extra */}
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#fff', borderRadius: 16, padding: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 4, elevation: 2 },
  ...
  saveBtn: { backgroundColor: '#2563EB', borderRadius: 14, paddingVertical: 15, alignItems: 'center', marginTop: 24, marginBottom: 8 },
